...
async def login(req: PasswordLoginRequest) -> dict[str, Any]:
    ...
    account = store.authenticate(req.username, req.password)
    if account is None:
    ...
    tokens = token_manager.create_token_set(
...
async def refresh_token(req: RefreshTokenRequest) -> dict[str, Any]:
    ...
    try:
        tokens = TokenManager().refresh_token_set(req.refresh_token)
        return success(
            data={
                "access_token": tokens.access_token,
                "refresh_token": tokens.refresh_token,
...
async def wecom_callback(req: WeComOAuthRequest) -> dict[str, Any]:
    ...
    try:
        tokens = await auth_service.verify_wecom_user(req)
        return success(
            data={
                "access_token": tokens.access_token,
                "refresh_token": tokens.refresh_token,
